...
const groupErrorsByJobAndStep = (errors: CIError[]): JobErrors[] => {
  ...
  for (const error of errors) {
    ...
    const step = error.workflowContext?.step ?? "Unknown step";
...
// ============================================================================
// Resolve Lifecycle Comments
// ============================================================================
// These comments are used during the resolve lifecycle:
// 1. Errors found → "Found X issues in Y jobs"
// 2. Resolve triggered → "Resolving X issues..."
// 3. Resolve success → "Resolved X issues. Ready to apply."
// 4. Resolve failed → "Failed to resolve: {reason}"
...
// Options for formatting a "resolve success" comment
export interface FormatResolveSuccessCommentOptions {
  filesFixed: number;
  projectUrl: string;
}

// Format comment when resolve completes successfully
export const formatResolveSuccessComment = (
  options: FormatResolveSuccessCommentOptions
): string => {
  const { filesFixed, projectUrl } = options;
  const fileText = filesFixed === 1 ? "1 file" : `${filesFixed} files`;

  const lines: string[] = [];
  lines.push(formatHeader(`Resolved ${fileText}. Ready to apply.`));
  lines.push("");
...
// Options for formatting a "resolve failed" comment
export interface FormatResolveFailedCommentOptions {
  reason: string;
}

// Format comment when resolve fails
export const formatResolveFailedComment = (
  options: FormatResolveFailedCommentOptions
): string => {
  ...
  const lines: string[] = [];
  lines.push(formatHeader("Failed to resolve."));
  lines.push("");
...
// Format comment when there are no resolve candidates
export const formatNoResolveCandidatesComment = (): string => {
