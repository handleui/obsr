...
// ============================================================================
// Resolve Creation Lock
// ============================================================================
// Prevents duplicate resolve creation for the same PR+source combination.
// This is necessary because webhooks can fire multiple times rapidly, and
// the database check for existing resolves has a race window.

const RESOLVE_CREATION_LOCK_PREFIX = "detent:resolve:create";

// Same TTL as commit locks (5 minutes)
const RESOLVE_CREATION_LOCK_TTL_SECONDS = 5 * 60;

// Stale threshold for resolve creation locks (2 minutes)
const RESOLVE_CREATION_LOCK_STALE_MS = 2 * 60 * 1000;

// Autofix source should be alphanumeric with optional hyphens/underscores
const AUTOFIX_SOURCE_REGEX = /^[a-zA-Z0-9_-]{1,50}$/;

interface ResolveCreationLockState {
  lockId: string;
  timestamp: number;
}

interface ResolveCreationLockResult {
  acquired: boolean;
  lockId?: string;
  kvError?: boolean;
  validationError?: boolean;
...
/**
 * Validates inputs for resolve creation lock operations.
 * Returns normalized project ID if valid, null otherwise.
 */
const validateResolveCreationInputs = (
  projectId: string,
  prNumber: number,
  autofixSource: string
): { projectId: string; prNumber: number; autofixSource: string } | null => {
  if (!projectId || typeof projectId !== "string") {
    console.warn("[resolve-creation-lock] Missing project ID");
    return null;
  }
  ...
  if (!trimmedProjectId || trimmedProjectId.length > 128) {
    console.warn(
      `[resolve-creation-lock] Invalid project ID format rejected: ${projectId.substring(0, 20)}...`
    );
    return null;
  }
  ...
  if (
    ...
    console.warn(
      `[resolve-creation-lock] Invalid PR number rejected: ${prNumber}`
    );
    return null;
  }
  ...
  if (!AUTOFIX_SOURCE_REGEX.test(autofixSource)) {
    console.warn(
      `[resolve-creation-lock] Invalid autofix source rejected: ${autofixSource.substring(0, 20)}...`
    );
    return null;
  }
...
const buildResolveCreationLockKey = (
  projectId: string,
  prNumber: number,
  autofixSource: string
): string =>
  `${RESOLVE_CREATION_LOCK_PREFIX}:${projectId}:${prNumber}:${autofixSource}`;

/**
 * Attempts to acquire a lock for creating a resolve for a specific PR+source.
 * This prevents duplicate resolve creation when multiple webhooks fire rapidly.
 *
 * Uses write-then-verify pattern to mitigate KV eventual consistency issues.
 */
export const acquireResolveCreationLock = async (
  kv: KVNamespace,
  projectId: string,
  prNumber: number,
  autofixSource: string
): Promise<ResolveCreationLockResult> => {
  const validated = validateResolveCreationInputs(
    projectId,
    prNumber,
    autofixSource
  );
  ...
  const key = buildResolveCreationLockKey(
    validated.projectId,
    validated.prNumber,
  ...
  try {
    const result = await tryAcquireLock<ResolveCreationLockState>(
      kv,
      key,
      RESOLVE_CREATION_LOCK_TTL_SECONDS,
      RESOLVE_CREATION_LOCK_STALE_MS,
      (lockId) => ({ lockId, timestamp: Date.now() }),
      "resolve-creation-lock"
    );
    ...
    if (result.acquired) {
      console.log(
        `[resolve-creation-lock] Acquired lock for project=${projectId} PR#${prNumber} source=${autofixSource}`
      );
    } else {
      console.log(
        `[resolve-creation-lock] Lock held for project=${projectId} PR#${prNumber} source=${autofixSource}, skipping`
      );
    }
    ...
    console.error(
      `[resolve-creation-lock] acquireResolveCreationLock failed for project=${projectId} PR#${prNumber} source=${autofixSource}, proceeding with fail-open:`,
      error
    );
    return { acquired: true, kvError: true };
  }
...
/**
 * Releases the resolve creation lock after resolve creation completes or fails.
 */
export const releaseResolveCreationLock = async (
  kv: KVNamespace,
  projectId: string,
  prNumber: number,
  autofixSource: string
  ...
  try {
    await kv.delete(key);
    console.log(
      `[resolve-creation-lock] Released lock for project=${projectId} PR#${prNumber} source=${autofixSource}`
    );
  } catch (error) {
    // Non-critical: TTL will clean up
    console.error(
      `[resolve-creation-lock] releaseResolveCreationLock failed for project=${projectId} PR#${prNumber} source=${autofixSource}:`,
      error
    );
  }
};
...
// ============================================================================
// Resolve Command Lock
// ============================================================================
// Prevents race conditions when two @detentsh resolve commands are posted
// simultaneously on the same PR. This ensures only one resolve orchestration
// runs at a time per PR, preventing duplicate resolves even when the
// existingResolves DB check has a race window.

const RESOLVE_COMMAND_LOCK_PREFIX = "detent:resolve:command";

// Short TTL since resolve orchestration should complete within 30 seconds
const RESOLVE_COMMAND_LOCK_TTL_SECONDS = 60;

// Stale threshold for resolve command locks (30 seconds)
const RESOLVE_COMMAND_LOCK_STALE_MS = 30 * 1000;

interface ResolveCommandLockState {
  lockId: string;
  timestamp: number;
}

interface ResolveCommandLockResult {
  acquired: boolean;
  lockId?: string;
  kvError?: boolean;
  validationError?: boolean;
...
/**
 * Validates inputs for resolve command lock operations.
 */
const validateResolveCommandInputs = (
  projectId: string,
  prNumber: number
): { projectId: string; prNumber: number } | null => {
  if (!projectId || typeof projectId !== "string") {
    console.warn("[resolve-command-lock] Missing project ID");
    return null;
  }
  ...
  if (!trimmedProjectId || trimmedProjectId.length > 128) {
    console.warn(
      `[resolve-command-lock] Invalid project ID format rejected: ${projectId.substring(0, 20)}...`
    );
    return null;
  }
  ...
  if (
    ...
    console.warn(
      `[resolve-command-lock] Invalid PR number rejected: ${prNumber}`
    );
    return null;
  }
...
const buildResolveCommandLockKey = (
  projectId: string,
  prNumber: number
): string => `${RESOLVE_COMMAND_LOCK_PREFIX}:${projectId}:${prNumber}`;

/**
 * Attempts to acquire a lock for the @detentsh resolve command on a specific PR.
 * This prevents race conditions when multiple resolve commands are posted
 * simultaneously, ensuring only one orchestration runs at a time.
 */
export const acquireResolveCommandLock = async (
  kv: KVNamespace,
  projectId: string,
  prNumber: number
): Promise<ResolveCommandLockResult> => {
  const validated = validateResolveCommandInputs(projectId, prNumber);
  if (!validated) {
    return { acquired: true, validationError: true };
  }

  const key = buildResolveCommandLockKey(
    validated.projectId,
    validated.prNumber
  );

  try {
    const result = await tryAcquireLock<ResolveCommandLockState>(
      kv,
      key,
      RESOLVE_COMMAND_LOCK_TTL_SECONDS,
      RESOLVE_COMMAND_LOCK_STALE_MS,
      (lockId) => ({ lockId, timestamp: Date.now() }),
      "resolve-command-lock"
    );
    ...
    if (result.acquired) {
      console.log(
        `[resolve-command-lock] Acquired lock for project=${projectId} PR#${prNumber}`
      );
    } else {
      console.log(
        `[resolve-command-lock] Lock held for project=${projectId} PR#${prNumber}, skipping`
      );
    }
    ...
    console.error(
      `[resolve-command-lock] acquireResolveCommandLock failed for project=${projectId} PR#${prNumber}, proceeding with fail-open:`,
      error
    );
    return { acquired: true, kvError: true };
  }
...
/**
 * Releases the resolve command lock after orchestration completes or fails.
 */
export const releaseResolveCommandLock = async (
  kv: KVNamespace,
  projectId: string,
  prNumber: number
): Promise<void> => {
  ...
  try {
    await kv.delete(key);
    console.log(
      `[resolve-command-lock] Released lock for project=${projectId} PR#${prNumber}`
    );
  } catch (error) {
    console.error(
      `[resolve-command-lock] releaseResolveCommandLock failed for project=${projectId} PR#${prNumber}:`,
      error
    );
  }
};
