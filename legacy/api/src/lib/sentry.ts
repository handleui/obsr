...
export interface LockConflictContext {
  lockType: "pr_comment" | "commit" | "resolve_creation";
  repository: string;
  prNumber?: number;
  deliveryId: string;
  operation: string;
 ...
 * These are non-blocking errors (resolves proceed without check runs) but
 ...
 * don't see the resolving status indicator on GitHub.
 *
 * @param error - The original error
...
export const captureCheckRunError = (
  ...
  Sentry.withScope((scope) => {
    ...
    scope.setTag("resolve.id", context.resolveId);
