...
import { soloInSet, toggleInSet } from "../lib/set-utils";
...
export const FilterProvider = ({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — stabilise identity via serialised key
  const allJobs = useMemo(() => jobList, [jobList.join("\0")]);
  const [jobs, setJobs] = useState(() => new Set(allJobs));
  ...
  const soloJob = useCallback(
    (value: string) => setJobs((prev) => soloInSet(prev, value, allJobs)),
    [allJobs]
  );
  ...
  const resetJobs = useCallback(() => setJobs(new Set(allJobs)), [allJobs]);
