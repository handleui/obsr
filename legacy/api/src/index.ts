import { scrubEvent } from "@obsr/sentry";
...
app.route("/v1/resolve/autofix-result", autofixResultRoutes);
...
const internalApi = new Hono<{ Bindings: Env }>();
internalApi.use("*", authMiddleware);
internalApi.use("*", rateLimitMiddleware);
internalApi.route("/auth", authRoutes);
internalApi.route("/resolve", resolveRoutes);
internalApi.route("/projects", projectsRoutes);
internalApi.route("/organization-members", organizationMembersRoutes);
internalApi.route("/organizations", organizationsRoutes);
internalApi.route("/invitations", invitationRoutes);
internalApi.route("/orgs/:orgId/invitations", orgInvitationsRoutes);
internalApi.route("/orgs", apiKeysRoutes);
internalApi.route("/orgs", orgWebhooksRoutes);
internalApi.route("/orgs", githubSecretsRoutes);
internalApi.route("/orgs", orgsByProviderRoutes);
internalApi.route("/billing", billingRoutes);

app.route("/v1", internalApi);
...
const worker: ExportedHandler<Env> = {
  ...
  async scheduled(_event, env, ctx) {
    ...
    const { cleanupStaleResolves } = await import(
      "./jobs/cleanup-stale-resolves"
    );

    ctx.waitUntil(
      Promise.all([
        syncAllOrganizations(env).catch((err: unknown) => {
          console.error("[scheduled] Sync failed:", err);
          Sentry.captureException(err);
        }),
        cleanupStaleResolves(env).catch((err: unknown) => {
          console.error("[scheduled] Cleanup stale resolves failed:", err);
          Sentry.captureException(err);
        }),
      ])
    );
