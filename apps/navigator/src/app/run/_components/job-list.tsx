...
const JobList = () => {
  ...
  const errorsByJob = useMemo(() => {
    const map = new Map<string, ErrorDetailData[]>();
    for (const e of errors) {
      const key = errorIdToJobKey(e.id);
  ...
  useEffect(() => {
    deselectMany(
      errors.filter((e) => !visible.has(errorIdToJobKey(e.id))).map((e) => e.id)
    );
  }, [visible, deselectMany, errors]);
