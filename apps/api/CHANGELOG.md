...
- 89239c2: Add `@detent resolve` command and move check runs to resolve trigger time.
  Check runs are no longer created automatically when CI starts. Instead, they are created when a user triggers resolving via the `@detent resolve` PR comment or dashboard. This reduces noise and gives users explicit control over when resolving happens.
...
- dbbee2f: Add billing enforcement to resolve creation requests to prevent unbilled resolves.
...
- e1cb50b: Replace log-based parsing with JSON-based error collection.
  ...
  Removes @obsr/parser dependency entirely. Error parsing now happens at the source via the new GitHub Action, which sends structured JSON to the /report endpoint. This eliminates regex-based log parsing in favor of consuming native JSON output from CI tools (ESLint, Vitest, golangci-lint, Cargo, TypeScript).
...
- 3855800: Add resolves layer for autofix orchestration with Modal executor integration.

  Introduces the `resolves` table to track autofix and AI resolve operations, organization settings for auto-commit behavior, KV-based deduplication locks, and a webhook endpoint for executor callbacks. Includes GitHub Data API integration for pushing commits directly to PR branches.
...
- Add API scaffold with Hono + Cloudflare Workers
  ...
  - Add `/v1/resolve` endpoint stub with SSE streaming (protected)
  - Add `X-API-Key` auth middleware (stub for WorkOS)
  - Add service wrappers for `@obsr/parser` and `@obsr/resolving`
