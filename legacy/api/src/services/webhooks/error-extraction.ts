import { DEFAULT_FAST_MODEL } from "@obsr/ai";
import { type createDb, runOps, storeJobReport } from "@obsr/db";
import {
...
import { canRunResolve, recordAIUsage } from "../billing";
...
const MAX_RESOLVES_PER_EXTRACTION = 10;
...
const resolveCreateStatus = async (
  env: Env,
  orgSettings: Required<OrganizationSettings>,
  organizationId: string
): Promise<ResolveCreateStatus> => {
  if (!orgSettings.resolveAutoTrigger) {
    return "found";
  }
  const billing = await canRunResolve(env, organizationId);
  return billing.allowed ? "pending" : "found";
};

interface DispatchResolvesOptions {
  env: Env;
  errorsByJob: Map<string, RunError[]>;
  existingSignatureIds: Set<string>;
  resolveStatus: ResolveCreateStatus;
  projectId: string;
  runRecordId: string;
  commitSha: string;
  prNumber: number | undefined;
...
const dispatchResolves = async (
  options: DispatchResolvesOptions
): Promise<void> => {
  const {
    env,
    errorsByJob,
  ...
  for (const [jobName, errors] of errorsByJob.entries()) {
    if (promises.length >= MAX_RESOLVES_PER_EXTRACTION) {
      console.warn(
        `${LOG_PREFIX} Hit max resolves limit (${MAX_RESOLVES_PER_EXTRACTION}), skipping remaining jobs`
      );
      break;
    }
    ...
    if (!hasUncoveredSignatures(signatureIds, existingSignatureIds)) {
      console.log(
        `${LOG_PREFIX} Skipping duplicate resolve for job "${jobName}" - signatures already covered`
      );
      continue;
    }
  ...
  if (promises.length > 0) {
    ...
    console.log(`${LOG_PREFIX} Created ${promises.length} resolve(s)`);
  }
};
...
const createResolvesForErrors = async (
  ...
  const [organization, resolvesByPr] = await Promise.all([
    ...
    prNumber
      ? getResolvesByPr(env, project._id, prNumber)
      : Promise.resolve([]),
  ]);
  ...
  const existingSignatureIds = new Set(
    resolvesByPr
      .filter((h) => h.type === "resolve")
      .flatMap((h) => h.signatureIds ?? [])
  );
  const errorsByJob = groupErrorsByWorkflowJob(runErrors);
  const resolveStatus = await resolveCreateStatus(
    env,
    orgSettings,
    project.organizationId
  );

  await dispatchResolves({
    env,
    errorsByJob,
    existingSignatureIds,
    resolveStatus,
...
const storeAndResolveErrors = async (
  pipeline: ExtractionPipelineContext
): Promise<void> => {
  ...
  try {
    ...
    console.error(
      `${LOG_PREFIX} ${ctx.logCtx}: Failed to create resolves:`,
...
export const extractAndStoreErrors = async (
  ...
  try {
    ...
    await storeAndResolveErrors(pipeline);
  } finally {
    await pool.end();
  }
};
