...
```
+------------------+     +------------------+     +------------------+
|     Client       |---->|    ResolveLoop      |---->|   ToolRegistry   |
...
- `"per-run"` - Single resolve exceeded budgetPerRunUSD
...
Checks extracted errors against current code before resolving. Errors may become stale if code changed between extraction and resolving.
...
|--------|--------|-------------|
| `successScore` | 0.50 | Did resolving succeed as expected? |
...
tracker.trackTaskCost(result.costUSD);    // ResolveLoop runs
...
export { ResolveLoop, createConfig } from "./loop.js";
