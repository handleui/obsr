...
describe("billing service", () => {
  ...
  // ==========================================================================
  // canRunResolve - billing check behavior
  // ==========================================================================

  describe("canRunResolve", () => {
    it("allows resolve when POLAR_ACCESS_TOKEN is not configured (dev mode)", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv({ POLAR_ACCESS_TOKEN: undefined });

      const result = await canRunResolve(env, "org-123");
    ...
    it("denies resolve when organization is not found", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv();
      ...
      const result = await canRunResolve(env, "nonexistent-org");
    ...
    it("denies resolve when customer not found in Polar", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv();
      mockQuery.mockImplementation((name: string) => {
        if (name === "organizations:getById") {
    ...
    it("allows resolve when customer has active subscription", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv();
      mockQuery.mockImplementation((name: string) => {
        if (name === "organizations:getById") {
    ...
    it("allows resolve when customer has meter credits", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv();
      mockQuery.mockImplementation((name: string) => {
        if (name === "organizations:getById") {
    ...
    it("denies resolve when no active subscription and no credits", async () => {
      const { canRunResolve } = await getBilling();
      const env = createBillingEnv();
      mockQuery.mockImplementation((name: string) => {
        if (name === "organizations:getById") {
