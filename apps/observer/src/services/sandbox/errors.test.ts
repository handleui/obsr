...
/** Regex to match trailing ellipsis */
const TRAILING_ELLIPSIS = /\.\.\.$/;

// Hoisted mocks - these must be hoisted to avoid reference errors
...
describe("truncateError", () => {
  ...
  it("truncates at exactly 200 char boundary", async () => {
    // Create message that is exactly at boundary
...
describe("missing API key detection", () => {
  ...
  it("allows service creation with valid API key", () => {
    const validEnv = createMockEnv("valid-key-123");

    expect(() => createSandboxService(validEnv)).not.toThrow();
  });
});
